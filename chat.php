<?php
    	...
    	if ($msg != "")
	{
        	...
        	if (strlen($msg) > 150)
		{
        	...
        	if (strlen($msg) > 15)
		{
            		if (substr_count($msg, substr($msg, 6, 8)) > 1)
			{
				die();
			}
        	}
        	...
        	foreach ($spam as $a)
		{
        	...
        	foreach ($espam as $a)
		{
            		if (strtoupper($msg) == strtoupper($a))
			{
				die();
			}
        	}
		...
		else
		{
        	...
        	if (substr_count($chattext, $msg) > 2)
		{
			die();
		}
        	...
        	if (strlen($n) > $nick_maxlength-1)
			$n = substr($n, 0, $nick_maxlength-1);

        	for ($i=0; $i<($nick_maxlength - strlen($n)); $i++)
			$spaces .= " ";
